...
mkdir -p /home/cccerith/.docker /home/cccerith/.kube /home/cccerith/.config/helm || true
...
if [[ -z "${HOST_HOME}" ]]; then
    ...
    if [[ ! -e "${HOST_HOME}/.kube/config" ]]; then
        ...
        sudo cp "${HOST_HOME}/.kube/config" /home/cccerith/.kube/config
        sudo chown cccerith:cccerith /home/cccerith/.kube/config
    fi

    echo "Setting up Helm..."

    HELM_REPOSITORIES_YAML=/home/cccerith/.config/helm/repositories.yaml
    ...
    if [[ -n "${HOST_HELM_REPOSITORIES_YAML}" ]]; then
        ...
        sudo chown cccerith:cccerith "${HELM_REPOSITORIES_YAML}"
    else
